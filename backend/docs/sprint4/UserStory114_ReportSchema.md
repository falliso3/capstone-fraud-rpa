...
| `total_transactions`            | total count of transactions in DB tied to this run         | alias for `rpa-runs.inserted` (until transactions are tied to runs)         |
| `flag_rate`                     | `flagged / scored` × 100 (%)                               | derived from `rpa_runs.flagged` and `rpa_runs.scored`                       |
| `avg_score`                     | average fraud score of all transactions in this run        | derived from `scores.score` filtered between `started_at` and `finished_at` |
